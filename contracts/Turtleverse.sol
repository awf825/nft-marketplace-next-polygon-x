...
contract Turtleverse is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier whenAnySaleActive() { require(giveawayActive || presaleActive || saleActive, "There are no sales at this time."); _; }
    ...
    function addToWhitelist(address[] memory addresses) external onlyOwner {
        require(addresses.length > 0, "Must send an array of addresses to whitelist");
    ...
    function removeFromWhitelist(address[] memory addresses) external onlyOwner {
        require(addresses.length > 0, "Must send an array of addresses to remove from whitelist");
        require(addresses.length <= 2000, "Whitelist cannot exceed 2000");
        for(uint index = 0; index < addresses.length; index+=1) {
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(totalSupply() < 10000, "No tokens left!");
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        require(_payout != address(0));
        return (_payout, (_salePrice * 1000) / 10000);
    } 
}